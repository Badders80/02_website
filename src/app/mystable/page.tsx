...
// Extended holding for subscription_float holdings
interface SubscriptionHoldingInfo {
  subscription_id: string | null;
  payment_style: string | null;
  list_rate_per_1pct_month: number | null;
  monthly_nzd: number | null;
  join_fee_nzd: number | null;
}
...
interface LiveHolding {
  ...
  kyc_status: string;
  // 017 subscription fields (from holdings table)
  subscription_id?: string | null;
  monthly_nzd?: number | null;
...
// Real-time subscription status fetched from Stripe via our API
  ...
  (hltsData as any[]).forEach((hlt: any) => {
    ...
    hltMap[key] = {
      ...
      horse: {
        ...
        sex: horse?.sex || "",
        colour: horse?.colour || "",
        sire_name: normalizePedigreeName(horse?.sire_name || ""),
        dam_name: normalizePedigreeName(horse?.dam_name || ""),
...
function liveToHoldingRecords(live: LiveHolding[], hltMap: Record<string, Campaign>): HoldingRecord[] {
  return live.map((h) => {
...
export default function MyStablePage() {
  ...
  // Subscription statuses — keyed by purchase_id
  const [subscriptionStatuses, setSubscriptionStatuses] = useState<Record<string, SubscriptionStatus>>({});
  const [subscriptionStatusesLoading, setSubscriptionStatusesLoading] = useState(false);

  // Portal loading state per holding
  ...
  // Fetch subscription statuses for subscription holdings
  const fetchSubscriptionStatuses = useCallback(async () => {
    const subHoldings = liveHoldings.filter(
      (h) => h.payment_style === "subscription_float" && h.subscription_id
    );
    if (subHoldings.length === 0) return;

    setSubscriptionStatusesLoading(true);
    const statuses: Record<string, SubscriptionStatus> = {};

    await Promise.all(
      subHoldings.map(async (h) => {
        try {
          const token = await user!.getIdToken();
          const res = await fetch(
            `/api/subscriptions/lookup?subscription_id=${h.subscription_id}`,
            { headers: { Authorization: `Bearer ${token}` } }
          );
          if (res.ok) {
            const data = await res.json();
            statuses[h.purchase_id] = data;
          }
        } catch {
          // Skip failed lookups
        }
    ...
    setSubscriptionStatusesLoading(false);
  }, [liveHoldings, user]);
  ...
  useEffect(() => {
    ...
    if (
      ...
      setTimeout(() => { fetchCommunications(); }, 0);
    }
  }, [activeTab, user, communications.length, communicationsError, communicationsLoading, fetchCommunications]);
  ...
  useEffect(() => {
    ...
    if (!user) {
      // Defer state clear to avoid React 19 strict-mode lint
      const id = setTimeout(() => {
        setHoldings([]);
        setCampaigns({});
        setUpdates([]);
        ...
        if (res.ok && Array.isArray(data.holdings)) {
          ...
          setErrorMsg(
            data?.code === "SHEETS_QUOTA"
              ...
              : data?.error || "Could not load live holdings. Refresh or try again shortly."
          );
        }
      } catch (err: any) {
  ...
  // Fetch subscription statuses after holdings load
  useEffect(() => {
    if (liveHoldings.length > 0 && !liveHoldingsLoading) {
      const id = setTimeout(() => void fetchSubscriptionStatuses(), 0);
      return () => clearTimeout(id);
    }
  }, [liveHoldings, liveHoldingsLoading, fetchSubscriptionStatuses]);
  ...
  // Separate subscription holdings from legacy
  const subscriptionHoldings = liveHoldings.filter(
    (h) => h.payment_style === "subscription_float"
  );
  const legacyHoldings = liveHoldings.filter(
    (h) => h.payment_style !== "subscription_float"
  );

  const handleBillingPortal = async (purchaseId: string) => {
    setPortalLoading(purchaseId);
    try {
      const token = await user!.getIdToken();
      const res = await fetch("/api/subscriptions/portal", {
        ...
        body: JSON.stringify({ return_url: `${window.location.origin}/mystable` }),
      });
      if (res.ok) {
        const data = await res.json();
        if (data.url) {
          // Navigate using the Router API to avoid React 19 immutability lint
  ...
  const handleCancelSubscription = async (subscriptionId: string, purchaseId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        void fetchSubscriptionStatuses();
      } else {
  ...
  return (
    <>
      <NavBar />
      <main className="dot-grid-surface min-h-screen bg-canvas text-foreground font-sans selection:bg-white/10 selection:text-pure-white relative">
        ...
        {!user && (
          <div className="absolute inset-0 z-50 flex items-center justify-center bg-canvas/45 backdrop-blur-[6px] px-6 py-20">
            <div className="rounded-3xl border border-border bg-[#0A0A0F]/65 backdrop-blur-2xl p-8 max-w-md w-full text-center space-y-6 shadow-[0_0_50px_rgba(0,0,0,0.85)] animate-fade-in pointer-events-auto">
              <div className="w-14 h-14 rounded-full bg-accent/10 border border-accent/20 flex items-center justify-center mx-auto text-2xl">
                🔑
              </div>
              ...
              <div className="space-y-4 pt-2">
                ...
                <Link
                  ...
                  className="block w-full text-center border border-border text-pure-white hover:bg-white/5 transition-all py-3.5 rounded-full text-[11px] font-medium uppercase tracking-widest active:scale-[0.98]"
                >
                  Sign In with Email
                </Link>
              </div>
        ...
        <div className={!user ? "blur-[12px] pointer-events-none select-none opacity-45 transition-all duration-700" : "transition-all duration-700"}>
          ...
          {purchaseSuccess && (
            <div className="px-12 md:px-16 lg:px-20 max-w-6xl mx-auto mb-8">
              <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-6 text-sm text-emerald-200/90 space-y-2">
                <p className="font-medium text-emerald-300">✅ Payment received</p>
                <p className="font-light text-emerald-200/70">
                  Your holding is being processed. If it is not listed yet, wait a few seconds and refresh.
                </p>
                <button
                  type="button"
                    ...
                    {communications.map((comm, idx) => {
                      ...
                      return (
                        <div key={key} className="relative space-y-3">
                          <span className="absolute -left-[31px] top-1.5 w-2 h-2 rounded-full bg-accent ring-4 ring-canvas" />
              ...
              <div className="space-y-6">
                ...
                {liveHoldingsLoading ? (
                  ...
                  <div className="space-y-4">
                    {liveHoldings.map((h) => {
                      const dsl = (hltsData as any[]).find((x) => (x.horse_slug || x.id) === h.horse_slug);
                      const horseName = dsl?.horse_name || h.horse_slug;
                      const hasDocs = h.signed_pds_url || h.signed_sa_url;
                      return (
                        <div key={h.purchase_id} className="rounded-xl border border-border bg-surface-base p-6 space-y-4">
                          <div className="flex items-center justify-between">
                            <div>
                              <h3 className="text-md font-medium text-heading">{horseName}</h3>
                              <p className="text-xs text-muted-foreground mt-0.5">
                                {h.payment_style === "subscription_float"
                                  ? `Subscription · ${h.shares_owned / 100}% stake`
                                  : `${h.shares_owned} shares · $${(h.purchase_price_total_nzd || 0).toLocaleString()} NZD`}
                              </p>
                            </div>
                            {!hasDocs && (
                              <span className="text-[10px] uppercase tracking-wider text-muted-foreground border border-border rounded-full px-3 py-1">
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-2">
                              ...
                              {h.signed_pds_url ? (
                                <a href={h.signed_pds_url} target="_blank" rel="noreferrer" className="text-xs text-accent hover:underline">View PDS ↗</a>
                              ) : (
                            ...
                            <div className="space-y-2">
                              ...
                              {h.signed_sa_url ? (
                                <a href={h.signed_sa_url} target="_blank" rel="noreferrer" className="text-xs text-accent hover:underline">View SA ↗</a>
                              ) : (
                                <p className="text-xs text-muted-foreground">Pending</p>
                              )}
                            </div>
          ...
          {activeTab === "overview" && (
            <section className="px-12 md:px-16 lg:px-20 max-w-6xl mx-auto pb-24">
              {errorMsg && (
                <div className="rounded-2xl border border-red-500/10 bg-red-500/5 p-6 mb-8 text-center text-sm font-light text-red-400">{errorMsg}</div>
              )}
              ...
              {loadingData || liveHoldingsLoading ? (
                  ...
                  <div className="flex flex-wrap gap-3 justify-center">
                    {liveHoldingsError && (
                      <button
                        type="button"
                        onClick={() => void fetchLiveHoldings()}
                        className="inline-block rounded-full border border-border text-pure-white px-8 py-3 text-[11px] font-medium uppercase tracking-widest hover:bg-white/5 transition-colors"
                      >
                        Retry
                      </button>
                    )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start">
                  {/* Left & Middle: Holdings */}
                  <div className="lg:col-span-2 space-y-12">
                    {/* Subscription Holdings */}
                    {subscriptionHoldings.length > 0 && (
                      <div className="space-y-6">
                        <div>
                          <h2 className="text-[20px] font-light text-heading mb-1">Active Subscriptions</h2>
                          <p className="text-xs font-light text-muted-foreground">Monthly syndicate stakes</p>
                        </div>
                        <div className="space-y-4">
                          {subscriptionHoldings.map((h) => {
                            const dsl = campaigns[h.horse_slug];
                            const horse = dsl?.horse;
                            const trainer = dsl?.trainer;
                            const subStatus = h.subscription_id ? subscriptionStatuses[h.purchase_id] : null;
                            const status = subStatus?.status || "active";
                            const nextBill = subStatus?.current_period_end
                              ? new Date(subStatus.current_period_end * 1000).toLocaleDateString("en-NZ", { day: "numeric", month: "short", year: "numeric" })
                              : null;
                            const pmInfo = subStatus?.payment_method
                              ? `${subStatus.payment_method.brand} ••••${subStatus.payment_method.last4}`
                              : null;

                            return (
                              <div key={h.purchase_id} className="group relative rounded-xl border border-border bg-surface-base p-6 hover:bg-surface-base transition-all duration-300">
                                <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-4">
                                  <div className="flex gap-4 items-center">
                                    <div className="w-12 h-12 rounded-xl bg-surface-base border border-border overflow-hidden relative flex-shrink-0">
                                      {horse?.image_url ? (
                                        ...
                                        <div className="flex h-full items-center justify-center text-muted-foreground text-[9px] font-light">N/A</div>
                                      )}
                                    </div>
                                    <div>
                                      <h3 className="text-md font-medium text-heading">{horse?.name || "Racehorse"}</h3>
                                      <p className="text-xs text-muted-foreground mt-0.5">
                                  ...
                                  <div className="flex sm:text-right flex-row sm:flex-col justify-between sm:justify-start gap-4">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-sm font-semibold text-pure-white">
                                        NZD${(h.monthly_nzd || 0).toLocaleString()}
                                      </p>
                                    </div>
                                  </div>
                                </div>
                                ...
                                <div className="flex flex-wrap gap-3 items-center pt-3 border-t border-border">
                                  <span className={`text-[10px] font-medium uppercase tracking-wider rounded-full px-2.5 py-1 border ${statusBadgeClass(status)}`}>
                                    {statusLabel(status)}
                                  </span>
                                  {nextBill && status === "active" && (
                                    <span className="text-[11px] font-light text-muted-foreground">
                                      Next bill: {nextBill}
                                    </span>
                                  )}
                                  {nextBill && status === "cancelling" && (
                                    <span className="text-[11px] font-light text-amber-400/70">
                                      Ends: {nextBill}
                                    </span>
                                  )}
                                  {pmInfo && status !== "cancelled" && (
                                    <span className="text-[11px] font-light text-muted-foreground">
                                      {pmInfo}
                                    </span>
                                  )}
                                ...
                                <div className="flex gap-3 pt-3">
                                  {status === "active" && h.subscription_id && (
                                    <>
                                      <button
                                        type="button"
                                        onClick={() => handleBillingPortal(h.purchase_id)}
                                        disabled={portalLoading === h.purchase_id}
                                        className="text-[11px] font-medium tracking-wider text-accent hover:text-pure-white transition disabled:opacity-50"
                                      >
                                        {portalLoading === h.purchase_id ? "Loading..." : "Update payment →"}
                                      </button>
                                      <button
                                        type="button"
                                        onClick={() => h.subscription_id && handleCancelSubscription(h.subscription_id, h.purchase_id)}
                                        disabled={cancelLoading === h.purchase_id}
                                        className="text-[11px] font-medium tracking-wider text-muted-foreground hover:text-red-400 transition disabled:opacity-50"
                                      >
                                        {cancelLoading === h.purchase_id ? "Loading..." : "Cancel subscription"}
                                      </button>
                                    </>
                                  )}
                                  <Link
                                    href={`/marketplace/${h.horse_slug}`}
                                    className="text-[11px] font-medium tracking-wider text-muted-foreground hover:text-pure-white transition ml-auto"
                                  >
                                    View horse →
                                  </Link>
                                </div>
                                ...
                                <p className="text-[10px] text-muted-foreground mt-3 pt-2 border-t border-border">
                                  Acquired {new Date(h.timestamp).toLocaleDateString("en-NZ", { year: "numeric", month: "short", day: "numeric" })}
                                </p>
                              </div>
                            );
                          })}
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="space-y-4">
                          {legacyHoldings.map((h) => {
                            ...
                            const horse = dsl?.horse;
                            const trainer = dsl?.trainer;
                            const holding = holdings.find((x) => x.hlt_id === h.horse_slug);
                            return (
                              <div key={h.purchase_id} className="group relative rounded-xl border border-border bg-surface-base p-6 hover:bg-surface-base transition-all duration-300">
                                <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-4">
                                  <div className="flex gap-4 items-center">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-sm font-semibold text-pure-white">
                                        ...
                                        <span className="text-muted-foreground font-normal text-xs ml-1">
                                          ({holding?.percentage_owned || 0}% of campaign)
                                        </span>
                                      </p>
                                    </div>
                                    <div>
                                      ...
                                      <p className="text-sm font-semibold text-pure-white">
                                        ${(h.purchase_price_total_nzd || 0).toLocaleString("en-NZ", { minimumFractionDigits: 0, maximumFractionDigits: 2 })} NZD
                                      </p>
                                    </div>
                                  </div>
                                </div>
                                <div className="pt-2 border-t border-border flex justify-between items-center">
                                  ...
                                  <Link href={`/marketplace/${h.horse_slug}`} className="text-[11px] text-accent hover:text-pure-white transition">
                                    View horse →
                                  </Link>
                                </div>
                              </div>
                        ...
                        <div className="space-y-8 relative border-l border-border pl-6 ml-3">
                          {updates.map((update) => (
                            <div key={update.id} className="relative space-y-3">
                              ...
                              <div className="flex justify-between items-start gap-4">
                                <div>
                                  <span className="text-[10px] font-medium uppercase tracking-wider text-accent">{update.horse_name}</span>
                                  <h4 className="text-md font-medium text-heading mt-1">{update.title}</h4>
                                </div>
                                <span className="text-xs font-light text-muted-foreground whitespace-nowrap">{update.content_date}</span>
                              </div>
                  ...
                  <div className="space-y-8">
                    <div className="rounded-2xl border border-border bg-surface-base p-6 space-y-1">
                      ...
                      <p className="text-xs text-muted-foreground font-light">
                        {subscriptionHoldings.length > 0
                          ? `${holdings.length} holding${holdings.length === 1 ? "" : "s"} (${subscriptionHoldings.length} subscription${subscriptionHoldings.length === 1 ? "" : "s"})`
                          : `Acquisition value across ${holdings.length} ${holdings.length === 1 ? "holding" : "holdings"}`}
                      </p>
                    </div>

                    {/* Quick Links */}
                    <div className="rounded-2xl border border-border bg-surface-base p-6 space-y-4">
                      <p className="text-[11px] font-light tracking-wider uppercase text-muted-foreground">Actions</p>
                      <div className="space-y-3 text-xs font-light text-foreground">
                        ...
                        {subscriptionHoldings.length > 0 && (
                          <button
                            type="button"
                            onClick={() => handleBillingPortal("all")}
                            disabled={portalLoading === "all"}
                            className="block w-full text-left hover:text-pure-white transition disabled:opacity-50"
                          >
                            {portalLoading === "all" ? "Loading..." : "Manage Billing (Stripe Portal) →"}
                          </button>
                        )}
